...
class TestAddLocationViewFormValid:
  ...
  def test_creates_location_with_correct_user(self, db):
    user = UserFactory()
    request = _post({'name': 'Camping Test'}, user)
  ...
  def test_sets_categories_when_submitted(self, db):
    ...
    request = _post({'name': 'Camping Test', 'categories': cat.slug}, user)
  ...
  def test_ignores_unknown_category_slugs(self, db):
    user = UserFactory()
    request = _post({'name': 'Camping Test', 'categories': 'does-not-exist'}, user)
