...
### Added
- **Tag deletion** from the staff dashboard — tags with zero published locations show a `×` delete button (browser confirm required). Deletion sets `status='x'` (soft-delete); tags still assigned to revoked locations are still deletable. Tags with active children are excluded from the card; a parent tag reappears automatically once all its children are soft-deleted (`Exists` subquery scoped to non-deleted children). Success message includes parent and tag name. ([#64](https://github.com/arnecoomans/cmpng/issues/64))
- **Tag usage card** on the staff dashboard — shows every published leaf tag with its published location count, a red *N revoked* badge linking to `?tag=<slug>&status=r` when relevant, and include/exclude links to the filtered list. Tags with zero published locations appear first. ([#64](https://github.com/arnecoomans/cmpng/issues/64))
- **Category usage card** on the staff dashboard — replaces the "fewest categories" list with a table showing every published category and how many published locations use it. Each row links to the filtered location list (include/exclude). Categories with zero locations are shown first. ([#63](https://github.com/arnecoomans/cmpng/issues/63))
...
- **Contact detail criterion for accommodations** — `phone` or `email` (at least one) is now a full base criterion for accommodations, worth 10 points against an expanded `applicable_max` of 105 (up from 95). Activities and other location types are unaffected. Accommodations missing a contact detail score ~8 points lower; those with one are net-neutral. `completeness_hints()` shows *Contact detail* for accommodations only. Run `python manage.py update_completeness` to backfill. ([#60](https://github.com/arnecoomans/cmpng/issues/60))
- **Completeness card** on the staff dashboard — merged with the existing "lowest completeness" list into a single card. Header shows a color-coded average badge. A Bootstrap stacked progress bar visualises the four score bands (red < 50, yellow 50–74, green 75–99, teal 100) with a legend row showing a dot + count per band. Location list appears below the legend, sorted lowest first. Computed via a single `aggregate()` call in a new `_completeness_context()` method on `StaffDashboardView`. ([#59](https://github.com/arnecoomans/cmpng/issues/59))
...
- Detail page map card and nearby/similar sections improved: map canvas reduced to 260px; Apple Maps / Google Maps / Waze navigation links added to the map card footer (shown when address is set); nearby/similar thumbnails replaced with visibility-aware `loc.thumb`; fold columns changed to 50/50. ([#55](https://github.com/arnecoomans/cmpng/issues/55))
