...
### Added
- Duplicate media uploads are now prevented: the upload button is disabled on submit and re-enabled on error (client-side); server-side, a SHA-256 hash of the file is stored on `Media` and duplicate uploads to the same location are silently dropped ([#23](https://github.com/arnecoomans/cmpng/issues/23))
